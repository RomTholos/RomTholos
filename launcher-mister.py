...
def play_rom(src_path):
    ...
    #result = subprocess.run(['ssh', f'{MISTER_USER}@{MISTER_IP}', cmd], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    print(result.stdout[:-1].decode("utf-8"))
    print(result.stderr[:-1].decode("utf-8"))
